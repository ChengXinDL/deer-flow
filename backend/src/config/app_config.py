...
class AppConfig(BaseModel):
    """Config for the magicflow application"""
    ...
    def resolve_config_path(cls, config_path: str | None = None) -> Path:
        ...
        2. If provided `MAGIC_FLOW_CONFIG_PATH` environment variable, use it.
        ...
        if config_path:
            ...
            return path
        elif os.getenv("MAGIC_FLOW_CONFIG_PATH"):
            path = Path(os.getenv("MAGIC_FLOW_CONFIG_PATH"))
            if not Path.exists(path):
                raise FileNotFoundError(f"Config file specified by environment variable `MAGIC_FLOW_CONFIG_PATH` not found at {path}")
            return path
        else:
...
def get_app_config() -> AppConfig:
    """Get the magicflow config instance.
