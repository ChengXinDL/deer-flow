...
magicflow 后端提供了完整的文件上传功能，支持多文件上传，并自动�?Office 文档�?PDF 转换�?Markdown 格式�?
## 功能特�?
- �?支持多文件同时上�?- �?自动转换文档�?Markdown（PDF、PPT、Excel、Word�?- �?文件存储在线程隔离的目录�?- �?Agent 自动感知已上传的文件
- �?支持文件列表查询和删�?
## API 端点
...
**路径说明�?*
- `path`: 实际文件系统路径（相对于 `backend/` 目录�?- `virtual_path`: Agent 在沙箱中使用的虚拟路�?- `artifact_url`: 前端通过 HTTP 访问文件�?URL
...
- 实际存储：`backend/.magic-flow/threads/{thread_id}/user-data/uploads/document.pdf`
- 前端访问：`/api/threads/{thread_id}/artifacts/mnt/user-data/uploads/document.pdf`（HTTP URL�?
上传流程采用“线程目录优先”策略：
- 先写�?`backend/magic-floww/threads/{thread_id}/user-data/uploads/` 作为权威存储
- 本地沙箱（`sandbox_id=local`）直接使用线程目录内�?- 非本地沙箱会额外同步�?`/mnt/user-data/uploads/*`，确保运行时可见
...
# 3. 列出已上传文�?curl http://localhost:2026/api/threads/test-thread/uploads/list
...
- 最大文件大小：100MB（可�?nginx.conf 中配�?`client_max_body_size`�?- 文件名安全性：系统会自动验证文件路径，防止目录遍历攻击
...
1. 检查文件大小是否超过限�?2. 检�?Gateway API 是否正常运行
3. 检查磁盘空间是否充�?4. 查看 Gateway 日志：`make gateway`

### 文档转换失败

1. 检�?markitdown 是否正确安装：`uv run python -c "import markitdown"`
2. 查看日志中的具体错误信息
3. 某些损坏或加密的文档可能无法转换，但原文件仍会保�?
### Agent 看不到上传的文件

1. 确认 UploadsMiddleware 已在 agent.py 中注�?2. 检�?thread_id 是否正确
3. 确认文件确实已上传到 `backend/.magic-flow/threads/{thread_id}/user-data/uploads/`
4. 非本地沙箱场景下，确认上传接口没有报错（需要成功完�?sandbox 同步�?
## 开发建�?
### 前端集成
...
1. **文件预览**：添加预览端点，支持在浏览器中直接查看文�?2. **批量删除**：支持一次删除多个文�?3. **文件搜索**：支持按文件名或类型搜索
4. **版本控制**：保留文件的多个版本
5. **压缩包支�?*：自动解�?zip 文件
6. **图片 OCR**：对上传的图片进�?OCR 识别
