# DeerFlow 前端 Demo 模式完整指南

## 概述

本文档记录了 DeerFlow 前端 Demo 模式的完整配置和使用方法，包括遇到的问题、解决方案以及最佳实践。这份文档的目标是让未来更容易配置和使用 Demo 模式。

## 目录

1. [Demo 模式简介](#demo-模式简介)
...
Demo 模式是 DeerFlow 前端的一种特殊运行模式，它允许前端应用在没有后端服务的情况下运行，使用预置的 Demo 数据来展示应用功能。

### Demo 模式的用途

- **功能演示**：向用户展示 DeerFlow 的各种功能
- **离线预览**：在没有后端服务的情况下预览前端界面
- **开发测试**：前端开发时不需要启动完整的后端服务
- **文档截图**：生成文档和演示材料
...
**关键点**：
- Demo 模式优先级最高
- 返回的路径不包含 `/api` 前缀（由 LangGraph SDK 自动添加）
- 需要同时处理客户端和服务端环境
...
export function urlOfArtifact({
  ...
  // 关键：Demo 模式下使用 LangGraph Base URL，而不是 Backend Base URL
...
**关键点**：
- 文件产物 URL 必须使用正确的 Base URL
- Demo 模式下使用 `getLangGraphBaseURL()`（返回 `/mock/api`）
- 正常模式下使用 `getBackendBaseURL()`（返回后端 API 地址）
- 路径格式：`{baseURL}/threads/{threadId}/artifacts{filepath}`
...
**关键点**：
- 必须同时支持 GET 和 POST 方法（LangGraph SDK 可能使用任一方法）
- 使用文件系统读取 Demo 数据
- 返回格式必须与真实 API 一致
...
**关键点**：
- 路径映射：`mnt/user-data/` → `public/demo/threads/{thread_id}/user-data/`
- 需要处理不同的文件类型（设置正确的 Content-Type）
- 支持下载参数（`?download=true`）
...
- 前端页面显示空白
- 网络请求返回 404 错误
- 日志显示请求 `/api/langgraph/threads/search` 而不是 `/mock/api/threads/search`
...
- 图片、文档等文件无法显示
- 网络请求返回 404 错误
- 日志显示请求 `/api/threads/{thread_id}/artifacts/...` 而不是 `/mock/api/threads/{thread_id}/artifacts/...`

**原因**：
- 文件产物 URL 构建逻辑未正确处理 Demo 模式
...
export function urlOfArtifact({ filepath, threadId, download = false }) {
  // 关键修改：Demo 模式下使用 getLangGraphBaseURL()
  const baseURL = env.NEXT_PUBLIC_STATIC_WEBSITE_ONLY === "true" 
    ? getLangGraphBaseURL()
    : getBackendBaseURL();
...
### 问题 3：Mock API 路由不支持 GET 方法

**症状**：
- 请求返回 405 Method Not Allowed
- LangGraph SDK 使用 GET 方法请求，但路由只支持 POST

**原因**：
- Mock API 路由只实现了 POST 方法
...
// 正确示例：
artifactPath.replace("mnt/", `public/demo/threads/${threadId}/user-data/`)
```
...
3. 添加文件产物到 `user-data/outputs/` 目录

4. 更新 `artifacts` 数组，包含所有文件路径

**注意事项**：
- `thread_id` 使用 UUID 格式
- 文件路径必须以 `mnt/user-data/outputs/` 开头
- `thread.json` 文件大小不要超过 20MB（Next.js 默认限制）
...
```typescript
// 在关键位置添加日志
console.log("Demo mode:", env.NEXT_PUBLIC_STATIC_WEBSITE_ONLY);
...
```typescript
// 验证文件路径，防止目录遍历攻击
function resolveArtifactPath(threadId: string, artifactPath: string): string {
...
1. **环境变量优先级**：`NEXT_PUBLIC_STATIC_WEBSITE_ONLY` > `NEXT_PUBLIC_LANGGRAPH_BASE_URL` > 默认配置
2. **API 路径构建**：Demo 模式使用 `/mock/api`，正常模式使用后端 URL
3. **文件产物 URL**：必须使用正确的 Base URL（Demo 模式下使用 `getLangGraphBaseURL()`）
4. **Mock API 路由**：必须同时支持 GET 和 POST 方法
5. **路径映射**：`mnt/user-data/` 映射到 `public/demo/threads/{thread_id}/user-data/`

### 常见错误

1. ❌ 文件产物 URL 使用了 `getBackendBaseURL()` 而不是 `getLangGraphBaseURL()`
2. ❌ Mock API 路由只实现了 POST 方法
3. ❌ 环境变量修改后未重启服务
4. ❌ 文件路径映射错误（缺少 `user-data/` 目录）
5. ❌ API 客户端缓存导致使用旧的配置
...
**API 客户端**：
- `frontend/src/core/api/api-client.ts` - LangGraph 客户端

**文件产物**：
- `frontend/src/core/artifacts/utils.ts` - URL 构建
- `frontend/src/core/artifacts/loader.ts` - 内容加载

**Mock API 路由**：
- `frontend/src/app/mock/api/threads/search/route.ts` - 线程搜索
...
**页面组件**：
- `frontend/src/app/workspace/page.tsx` - 工作空间主页
