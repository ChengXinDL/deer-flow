...
基于 FastAPI 的 REST API，提供非 Agent 操作的端点。
...
│  │ 4. DanglingToolCallMiddleware - Fix tool call responses         │   │
...
扩展 LangGraph 的 `AgentState`，添加 DeerFlow 特有字段：

```python
class ThreadState(AgentState):
    messages: list[BaseMessage]      # 核心状态
    sandbox: dict                    # 沙箱环境信息
...
│ - task (subagent)   │  │ - read_file         │  │ - brave-search      │
│                     │  │ - write_file        │  │                     │
│                     │  │ - str_replace       │  │                     │
└─────────────────────┘  └─────────────────────┘  └─────────────────────┘
```
...
- **防抖更新**: 批处理更新以最小化 LLM 调用
- **系统提示注入**: 将顶级事实 + 上下文注入 Agent 提示
